...
public class StatutController {
    ...
    public Response<StatutDto> create(@RequestBody Request<StatutDto> request) {
    	slf4jLogger.info("start method /statut/create");
        ...
        try {
        	...
        	if(!response.isHasError()){
               response = statutBusiness.create(request, locale);
        	}else{
    ...
    public Response<StatutDto> update(@RequestBody Request<StatutDto> request) {
		...
		try {
        	...
        	response = Validate.validateList(request, response, functionalError, locale);
        	if(!response.isHasError()){
               response = statutBusiness.update(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
    ...
    public Response<StatutDto> delete(@RequestBody Request<StatutDto> request) {
        ...
        try {
        	...
        	response = Validate.validateList(request, response, functionalError, locale);
        	if(!response.isHasError()){
               response = statutBusiness.delete(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
    ...
    public Response<StatutDto> getByCriteria(@RequestBody Request<StatutDto> request) {
        ...
        try {
        	...
        	if(!response.isHasError()){
               response = statutBusiness.getByCriteria(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
		...
		slf4jLogger.info("end method /statut/getByCriteria");
        return response;
    }
}
