...
public class ProfilController {
    ...
    public Response<ProfilDto> create(@RequestBody Request<ProfilDto> request) {
    	slf4jLogger.info("start method /profil/create");
        ...
        try {
        	...
        	if(!response.isHasError()){
               response = profilBusiness.create(request, locale);
        	}else{
    ...
    public Response<ProfilDto> update(@RequestBody Request<ProfilDto> request) {
		...
		try {
        	...
        	response = Validate.validateList(request, response, functionalError, locale);
        	if(!response.isHasError()){
               response = profilBusiness.update(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
    ...
    public Response<ProfilDto> delete(@RequestBody Request<ProfilDto> request) {
        ...
        try {
        	...
        	response = Validate.validateList(request, response, functionalError, locale);
        	if(!response.isHasError()){
               response = profilBusiness.delete(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
    ...
    public Response<ProfilDto> getByCriteria(@RequestBody Request<ProfilDto> request) {
        ...
        try {
        	...
        	if(!response.isHasError()){
               response = profilBusiness.getByCriteria(request, locale);
        	}else{
        	   slf4jLogger.info("Erreur| code: {} -  message: {}", response.getStatus().getCode(), response.getStatus().getMessage());
        	   return response;
        	}
		...
		slf4jLogger.info("end method /profil/getByCriteria");
        return response;
    }
}
