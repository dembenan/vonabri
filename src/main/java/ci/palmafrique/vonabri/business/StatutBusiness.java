...
public class StatutBusiness implements IBasicBusiness<Request<StatutDto>, Response<StatutDto>> {
	...
	@Override
	public Response<StatutDto> create(Request<StatutDto> request, Locale locale)  {
		log.info("----begin create Statut-----");
		
		response = new Response<StatutDto>();
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.CREATE_STATUT.getValue(), locale);
	...
	@Transactional(rollbackFor = { RuntimeException.class, Exception.class })
	@Override
	public Response<StatutDto> update(Request<StatutDto> request, Locale locale)  {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.UPDATE_STATUT.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
	...
	public Response<StatutDto> delete(Request<StatutDto> request, Locale locale)  {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.DELETE_STATUT.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
			...
			for (StatutDto dto : request.getDatas()) {
				...
				// travailleur
				List<Travailleur> listOfTravailleur = travailleurRepository.findByStatutId(existingEntity.getId(), false);
	...
	@SuppressWarnings("unused")
	@Override
	public Response<StatutDto> getByCriteria(Request<StatutDto> request, Locale locale) {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.VIEW_STATUT.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
			...
			List<Statut> items = null;
			items = statutRepository.getByCriteria(request, em, locale);
