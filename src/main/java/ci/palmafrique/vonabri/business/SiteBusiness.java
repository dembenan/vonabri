...
public class SiteBusiness implements IBasicBusiness<Request<SiteDto>, Response<SiteDto>> {
	...
	public Response<SiteDto> create(Request<SiteDto> request, Locale locale)  {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.CREATE_SITE.getValue(), locale);
			...
			for (SiteDto dto : request.getDatas()) {
				...
				existingEntity = siteRepository.findByCode(dto.getCode(), false);
				if (existingEntity != null) {
					response.setStatus(functionalError.DATA_EXIST("site code -> " + dto.getCode(), locale));
					response.setHasError(true);
					return response;
				}
	...
	public Response<SiteDto> update(Request<SiteDto> request, Locale locale)  {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.UPDATE_SITE.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
	...
	public Response<SiteDto> delete(Request<SiteDto> request, Locale locale)  {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.DELETE_SITE.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
			...
			for (SiteDto dto : request.getDatas()) {
				...
				// sousSite
				List<SousSite> listOfSousSite = sousSiteRepository.findBySiteId(existingEntity.getId(), false);
				if (listOfSousSite != null && !listOfSousSite.isEmpty()){
					response.setStatus(functionalError.DATA_NOT_DELETABLE("(" + listOfSousSite.size() + ")", locale));
					response.setHasError(true);
					return response;
				}
				// travailleur
				List<Travailleur> listOfTravailleur = travailleurRepository.findBySiteId(existingEntity.getId(), false);
	...
	public Response<SiteDto> getByCriteria(Request<SiteDto> request, Locale locale) {
		...
		try {
			...
			Response<UserDto> userResponse = userBusiness.isGranted(request, FunctionalityEnum.VIEW_SITE.getValue(), locale);
			if (userResponse.isHasError()) {
				response.setHasError(true);
				response.setStatus(userResponse.getStatus());
				return response;
